...
function MessageBubble({
  ...
  const isAudio = useMemo(() => {
    ...
    const url = (message.image_url || "").toLowerCase();
    return /\.(mp3|wav|ogg|webm|m4a|aac|opus)$/i.test(url);
  }, [message.e2ee?.media?.kind, message.image_url]);
  const isFile = mediaKind === "file" && !isAudio;
  ...
  return (
    <>
      <div
        id={`msg-${message.id}`}
        className={`w-full flex items-center transition-all duration-200 ${selected ? "bg-[rgb(var(--accent-500-rgb)/0.12)] px-4 py-2" : "px-4 py-1"} ${centerAligned ? "justify-center" : (mine ? "justify-end" : "justify-start")}`}
        onClick={(event) => {
          if (isSelectionMode) {
            event.preventDefault();
      ...
      >
        {!centerAligned && isSelectionMode && (
          <div className="flex items-center justify-center mr-3 cursor-pointer self-center shrink-0">
            <div className={`h-5 w-5 rounded-full border-2 flex items-center justify-center transition-all ${selected ? "border-[rgb(var(--accent-500-rgb))] bg-[rgb(var(--accent-500-rgb))] text-white" : "border-slate-300 dark:border-slate-600 bg-transparent"}`}>
        ...
        >
          ...
          <div className="relative chat-message-in">
            <div
            ...
            {message.reply_to_message_id && !deleted && (
              <div
                ...
                className={`mb-1.5 cursor-pointer rounded-lg border-l-2 px-2.5 py-1.5 text-[11px] transition-all hover:bg-opacity-25 active:scale-[0.99] select-none ${
                  mine
                    ? "border-white/80 bg-white/18 hover:bg-white/25 text-white"
                    : "border-accent bg-accent-soft hover:bg-accent-soft/80 text-slate-700 dark:bg-slate-800/85 dark:text-slate-200 dark:hover:bg-slate-800"
                }`}
              >
            ...
            {message.image_url && isAudio && !deleted && (
              encryptedMedia && !src ? (
                <div className="mt-2 w-full max-w-[340px] rounded-xl border border-black/10 bg-black/5 px-3 py-2 text-[11px] text-slate-700 dark:border-white/10 dark:bg-white/5 dark:text-slate-200">
                  {decryptingMedia ? "Decrypting voice note..." : "Unable to decrypt voice note."}
                </div>
              ) : (
                <div className="mt-2 w-full max-w-[340px]">
                  <AudioMessagePlayer src={src || cipherSrc} />
                </div>
              )
            )}
          ...
          {menuOpen && typeof document !== "undefined" && createPortal((
            <>
              ...
              <div
                ref={mobileMenuRef}
                onContextMenu={(event) => event.preventDefault()}
                className="fixed inset-x-0 bottom-0 z-[120] max-h-[74vh] overflow-y-auto overscroll-contain rounded-t-3xl border-t border-slate-200 bg-white px-5 pt-3 pb-[calc(1.25rem+env(safe-area-inset-bottom))] shadow-2xl dark:border-slate-800 dark:bg-slate-950 sm:hidden"
              >
                {/* Drag handle / Pull bar */}
                <div className="mx-auto mb-3 h-1 w-10 rounded-full bg-slate-200 dark:bg-slate-800" />
  ...
  return (
    <div className="flex items-center gap-3 w-full min-w-[220px] bg-black/10 dark:bg-white/10 rounded-2xl p-3 select-none">
      ...
      <button
        type="button"
        onClick={togglePlay}
        className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-accent text-white shadow hover:scale-105 active:scale-95 transition-all duration-150"
      >
