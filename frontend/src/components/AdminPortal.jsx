...
const panelClass = "rounded-2xl border border-amber-900/15 bg-slate-900/40 backdrop-blur-xl shadow-2xl transition hover:border-amber-900/25 duration-300";
const inputClass = "w-full rounded-xl border border-slate-800/80 bg-slate-950/60 px-4 py-2.5 text-xs text-slate-100 outline-none transition placeholder:text-slate-500 focus:border-amber-500 focus:ring-4 focus:ring-amber-500/10";
const subtleBtnClass = "inline-flex items-center justify-center gap-2 rounded-xl border border-slate-800 bg-slate-900/60 px-4 py-2.5 text-xs font-semibold text-slate-200 transition hover:border-amber-700/40 hover:bg-slate-800/80 hover:text-amber-200 shadow-sm active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-60";
const cyanBtnClass = "inline-flex items-center justify-center gap-2 rounded-xl border border-amber-500/40 bg-gradient-to-r from-amber-600/20 to-yellow-600/20 px-4 py-2.5 text-xs font-semibold text-amber-200 hover:text-white transition hover:from-amber-600/30 hover:to-yellow-600/30 shadow-md active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-60";
...
function activityTypeMeta(type) {
  ...
  if (normalized === "PROFILE_UPDATE") {
    return {
      ...
      badgeClass: "border-amber-500/55 bg-amber-500/15 text-amber-100"
    };
  }
...
export default function AdminPortal() {
  ...
  function deleteUser(user) {
    setConfirmConfig({
      title: "Delete User",
      message: `Delete user "${user.name}" permanently?`,
      onConfirm: async () => {
  ...
  function deleteAdmin(item) {
    if (!isSuperAdmin) return;
    setConfirmConfig({
      title: "Delete Admin",
      message: `Delete admin "${item.username}"?`,
      onConfirm: async () => {
        setError("");
        setNotice("");
                  ...
                  {admins.map((item, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-4 py-3.5">
                          <span className="font-medium text-amber-200">@{item.username}</span>
                        </td>
                        <td className="px-4 py-3.5">
                        ...
                        <td className="px-4 py-3.5">
                          <div className="flex flex-wrap gap-2">
                            <button
                              ...
                              disabled={!isSuperAdmin || busyAdminId !== null}
                              className="rounded-lg border border-amber-500/55 bg-amber-500/15 px-2.5 py-1.5 text-xs font-semibold text-amber-100 transition hover:bg-amber-500/25 disabled:cursor-not-allowed disabled:opacity-60"
                            >
                              Edit
                            </button>
                            <button
                ...
                <article key={item.id} className="rounded-2xl border border-slate-700/80 bg-slate-900/70 p-3.5 shadow-[0_14px_26px_-20px_rgb(0_0_0_/_0.95)]">
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      ...
                      <p className="truncate text-xs text-amber-300">@{item.username}</p>
                    </div>
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-2">
                    <button
                      ...
                      onClick={() => startEditAdmin(item)}
                      disabled={!isSuperAdmin || busyAdminId !== null}
                      className="rounded-lg border border-amber-500/55 bg-amber-500/15 px-2 py-1.5 text-xs font-semibold text-amber-100 transition hover:bg-amber-500/25 disabled:cursor-not-allowed disabled:opacity-60"
                    >
                      Edit
                    </button>
                    <button
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden bg-gradient-to-br from-slate-950 via-amber-950/25 to-slate-950 text-slate-100 font-sans">
      {/* Sidebar for desktop */}
      <aside className="hidden lg:block w-[290px] h-full shrink-0">
        <Sidebar
          admin={admin}
          tab={tab}
          onTabChange={(nextTab) => {
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 h-14 z-40 bg-slate-950/80 backdrop-blur-md border-b border-slate-900/50 flex items-center justify-between px-4">
      ...
      {sidebarOpen && (
        <div className="fixed inset-0 z-50 bg-slate-950/75 p-3 lg:hidden flex justify-start">
          <Sidebar
            admin={admin}
            tab={tab}
            mobile
      ...
      {/* Main content area */}
      <main className="flex-1 h-full overflow-y-auto p-4 lg:p-6 space-y-4 pt-18 lg:pt-6">
