  ...
  return (
    <div className="p-6 space-y-6 overflow-y-auto">
      ...
      <div className="flex space-x-1 bg-slate-100 dark:bg-neutral-800 p-1 rounded-lg">
        {['overview', 'connections', 'events', 'errors', 'users'].map((tab) => (
          <button
            key={tab}
      ...
      {activeTab === 'overview' && monitoringData && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="bg-white dark:bg-black rounded-lg border border-slate-200 dark:border-neutral-800 p-4">
            <div className="text-sm text-slate-500 dark:text-neutral-400">Realtime Status</div>
            <div className={`text-2xl font-bold ${monitoringData.socketStatus === 'online' ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400'}`}>
              {monitoringData.socketStatus === 'online' ? 'Connected' : 'Disconnected'}
            </div>
          </div>
          <div className="bg-white dark:bg-black rounded-lg border border-slate-200 dark:border-neutral-800 p-4">
      ...
      {activeTab === 'connections' && monitoringData?.activeSockets && (
        <div className="bg-white dark:bg-black rounded-lg border border-slate-200 dark:border-neutral-800">
          ...
          <div className="divide-y divide-slate-200 dark:divide-neutral-800 max-h-[60vh] overflow-y-auto">
      ...
      {activeTab === 'errors' && monitoringData?.recentErrors && (
        <div className="bg-white dark:bg-black rounded-lg border border-slate-200 dark:border-neutral-800">
          <div className="p-4 border-b border-slate-200 dark:border-neutral-800">
            <h3 className="text-lg font-semibold text-slate-900 dark:text-slate-100">Backend Error Console</h3>
          </div>
          <div className="divide-y divide-slate-200 dark:divide-neutral-800 max-h-96 overflow-y-auto">
            {monitoringData.recentErrors.map((error) => (
              <div key={error.id || error.timestamp} className="p-4">
                <div className="flex justify-between items-start gap-4">
                  <div className="flex-1">
                    <div className="font-medium text-red-600 dark:text-red-400">{error.type || 'error'}</div>
                    <div className="text-sm text-slate-600 dark:text-slate-400 mt-1">
                      {error.message}
                    </div>
                    {error.path && (
            ...
            {!monitoringData.recentErrors.length && (
              <div className="p-4 text-center text-slate-500 dark:text-neutral-500">
