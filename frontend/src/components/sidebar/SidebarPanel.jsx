...
export default function SidebarPanel({
  ...
  const handleStartChatClick = async (personId) => {
    setStartingChatId(personId);
    try {
      await onStartChat?.(personId);
    } finally {
  ...
  const [archivedChatIds, setArchivedChatIds] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem(`ana_archived_chats_${me?.id || "guest"}`) || "[]");
    } catch {
      return [];
    }
  });
  const [favouriteChatIds, setFavouriteChatIds] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem(`ana_favourite_chats_${me?.id || "guest"}`) || "[]");
    } catch {
      return [];
    }
  });
  ...
  useEffect(() => {
    const handleUpdate = () => {
      try {
        const archived = JSON.parse(localStorage.getItem(`ana_archived_chats_${me?.id || "guest"}`) || "[]");
        setArchivedChatIds(archived);
      } catch (e) {}
      try {
        const favourites = JSON.parse(localStorage.getItem(`ana_favourite_chats_${me?.id || "guest"}`) || "[]");
        setFavouriteChatIds(favourites);
      } catch (e) {}
    };
    window.addEventListener("ana_chats_updated", handleUpdate);
    return () => window.removeEventListener("ana_chats_updated", handleUpdate);
  }, [me?.id]);
  ...
  useEffect(() => {
    ...
    fetchStatuses();
    window.addEventListener("anach_status_updated", fetchStatuses);
    return () => {
      window.removeEventListener("anach_status_updated", fetchStatuses);
    };
  }, []);
  ...
  const filteredChats = useMemo(() => {
    ...
    if (resolvedActiveFilter === "archived") {
      return list.filter((chat) => archivedChatIds.includes(chat.id));
    }

    // Exclude archived chats from all other views/filters
    const nonArchivedList = list.filter((chat) => !archivedChatIds.includes(chat.id));

    if (resolvedActiveFilter === "all") return nonArchivedList;
    ...
    if (!custom) return nonArchivedList;
    const selectedIds = new Set(custom.chatIds.map(Number));
    return nonArchivedList.filter((chat) => selectedIds.has(Number(chat.id)));
  }, [resolvedActiveFilter, chats, customFilters, archivedChatIds, favouriteChatIds]);
  ...
  const combinedFeeds = useMemo(() => {
    const list = [...dbStatuses];
    MOCK_CONTACTS_STATUSES.forEach((mock) => {
      if (!list.some((feed) => feed.name.toLowerCase() === mock.name.toLowerCase() || feed.userId === mock.id)) {
        list.push({
          id: mock.id,
          userId: mock.id,
          name: mock.name,
  ...
  const myStatusFeed = useMemo(() => {
    const found = combinedFeeds.find((feed) => Number(feed.userId) === Number(me?.id));
  ...
  }, [combinedFeeds, me, anaUpdates]);

  const contactFeeds = useMemo(() => {
    return combinedFeeds.filter((feed) => Number(feed.userId) !== Number(me?.id));
  }, [combinedFeeds, me]);
  ...
  function handlePickedImage(file) {
    ...
    reader.onload = () => {
      ...
      onSelectStatusFeed?.({
        ...
        items: [],
        isCreator: true,
        initialMedia: { type: "image", dataUrl, name: file.name }
      });
    };
  ...
  function handlePickedVideo(file) {
    ...
    onSelectStatusFeed?.({
      ...
      items: [],
      isCreator: true,
      initialMedia: { type: "video", url, name: file.name }
    });
  }
  ...
  return (
    <>
      <aside
        className={`relative flex h-full min-h-0 min-w-0 flex-col overflow-hidden rounded-none border-r border-slate-200/80 bg-white shadow-none sm:rounded-none dark:border-slate-800 dark:bg-slate-950 ${
      ...
      >
        ...
        >
          <div
            className={`relative grid grid-cols-3 rounded-xl p-0.5 transition-all ${
              hasCustomSidebarColor
                ? (isSidebarLight ? "bg-black/5" : "bg-white/10")
          ...
          >
            {(() => {
              ...
              return (
                <div
                  aria-hidden
                  className={`pointer-events-none absolute inset-y-0.5 left-0.5 rounded-lg transition-all duration-300 ease-[cubic-bezier(0.34,1.56,0.64,1)] ${
                    hasCustomSidebarColor
                      ? (isSidebarLight ? "bg-white shadow-[0_2px_6px_rgba(0,0,0,0.06)]" : "bg-white/15")
                      : "bg-white shadow-[0_2px_8px_rgba(0,0,0,0.08)] dark:bg-slate-800 dark:shadow-[0_2px_8px_rgba(0,0,0,0.2)]"
                  }`}
                  style={{
              ...
              {!searchingPeople && peopleResults.map((person) => {
                ...
                return (
                  <button
                    key={person.id}
                    type="button"
                    onClick={() => handleStartChatClick(person.id)}
                    disabled={startingChatId !== null}
                    className="flex w-full items-center justify-between rounded-xl border border-slate-200 bg-white px-2.5 py-2 text-left transition hover:border-violet-300 hover:bg-violet-50 dark:border-slate-700 dark:bg-slate-900 dark:hover:border-violet-700 dark:hover:bg-slate-800 disabled:opacity-75 disabled:cursor-not-allowed"
                  >
          ...
          >
            ...
            <section className="min-w-full flex min-h-0 flex-1 flex-col">
              ...
              <div className="px-4 py-2 flex items-center justify-between">
                <p className={`text-[11px] font-semibold uppercase tracking-wide ${sidebarLabelClass}`}>Status Updates</p>
              </div>

              {/* Status Filter Pills */}
              <div className="px-4 pb-2 flex gap-1.5 overflow-x-auto scrollbar-hide">
                {["all", "personal", "group", "list"].map((pill) => (
                  <button
                    key={pill}
                    type="button"
                    onClick={() => setStatusFilter(pill)}
                    className={`inline-flex items-center gap-1.5 whitespace-nowrap rounded-full border px-2.5 py-1 text-[11px] font-semibold transition capitalize ${
                      statusFilter === pill
                        ? "border-violet-400 bg-violet-500/10 text-violet-600 dark:text-violet-400"
                        : "border-slate-200 bg-white text-slate-500 hover:border-slate-350 dark:border-slate-800 dark:bg-slate-900/50 dark:text-slate-400"
                    }`}
                  >
                    {pill}
                  </button>
              ...
              {/* Status List Scroll area */}
              <div className="chat-scroll min-h-0 flex-1 overflow-y-auto px-2 pb-[max(4.25rem,env(safe-area-inset-bottom))] space-y-4">
                
                {/* My Status Card */}
                {(statusFilter === "all" || statusFilter === "personal") && (
                  <div 
                    onClick={() => {
                      if (myStatusFeed.items && myStatusFeed.items.length > 0) {
                        onSelectStatusFeed?.({
                          name: me?.name || "My Status",
                          avatar: me?.avatar_url,
                          isMe: true,
                          items: myStatusFeed.items
                        });
                      } else {
                        onSelectStatusFeed?.({
                          name: me?.name || "My Status",
                    ...
                    className="flex items-center justify-between p-2.5 rounded-xl border border-slate-150 dark:border-slate-900/40 bg-slate-50/50 dark:bg-slate-900/20 hover:bg-slate-100/50 dark:hover:bg-slate-900/40 cursor-pointer transition"
                  >
                    <div className="flex items-center gap-3">
                      <div className="relative">
                        <div className={`rounded-full p-[2px] transition ${myStatusFeed.items && myStatusFeed.items.length > 0 ? "ring-2 ring-emerald-500 ring-offset-2 dark:ring-offset-slate-950" : ""}`}>
                          <Avatar name={me?.name || "Me"} src={me?.avatar_url} size={42} className="shadow-sm" />
                        </div>
                        {(!myStatusFeed.items || myStatusFeed.items.length === 0) && (
                          <span className="absolute bottom-0 right-0 inline-flex h-4 w-4 items-center justify-center rounded-full bg-violet-655 text-white ring-2 ring-white dark:ring-slate-950">
                            <Plus size={10} />
                          </span>
                        )}
                      </div>
                      <div className="min-w-0">
                        <p className="text-xs font-bold text-slate-900 dark:text-slate-105">My Status</p>
                        <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-0.5 truncate">
                          {myStatusFeed.items && myStatusFeed.items.length > 0 
                            ? (myStatusFeed.items[0].text || "Shared a status update")
                ...
                <div className="space-y-3">
                  {/* Recent Updates */}
                  {(() => {
                    const filteredList = contactFeeds.filter(feed => {
                      if (statusFilter === "all") return true;
                      if (statusFilter === "personal") return feed.chat_type === "personal" || !feed.chat_type;
                      if (statusFilter === "group") return feed.chat_type === "group";
                      return false;
                    });
 
                    const recentUpdates = filteredList.filter(item => item.unseen);
                    const viewedUpdates = filteredList.filter(item => !item.unseen);
 
                    return (
                      <>
                        {recentUpdates.length > 0 && (
                          <div className="space-y-1.5">
                            <p className="px-2 text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-slate-505">
                              Recent Updates
                            </p>
                            {recentUpdates.map((feed) => (
                              <div
                                key={feed.id}
                                ...
                                className="flex items-center gap-3 p-2 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-900/40 cursor-pointer transition animate-in fade-in"
                              >
                                <div className="rounded-full p-[2px] border-2 border-violet-500 shadow-sm">
                                  <Avatar name={feed.name} src={feed.avatar} size={38} />
                                </div>
                                <div className="min-w-0 flex-1">
                                  <p className="text-xs font-bold text-slate-800 dark:text-slate-200">{feed.name}</p>
                                  <p className="text-[10px] text-slate-400 dark:text-slate-505 mt-0.5">{feed.time || "Recently"}</p>
                                </div>
                        ...
                        {viewedUpdates.length > 0 && (
                          <div className="space-y-1.5 pt-2">
                            <p className="px-2 text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-slate-550">
                              Viewed Updates
                            </p>
                            {viewedUpdates.map((feed) => (
                              <div
                                key={feed.id}
                                onClick={() => onSelectStatusFeed?.({
                                  name: feed.name,
                                ...
                                className="flex items-center gap-3 p-2 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-900/40 cursor-pointer opacity-70 hover:opacity-100 transition"
                              >
                                <div className="rounded-full p-[2px] border-2 border-slate-300 dark:border-slate-700 shadow-sm">
                                  <Avatar name={feed.name} src={feed.avatar} size={38} />
                                </div>
                                <div className="min-w-0 flex-1">
                                  <p className="text-xs font-bold text-slate-800 dark:text-slate-200">{feed.name}</p>
                                  <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-0.5">{feed.time || "Viewed"}</p>
                                </div>
                              </div>
                            ))}
                          </div>
      ...
      <input
        ...
        accept="image/*"
        className="hidden"
        onChange={(e) => handlePickedImage(e.target.files?.[0])}
      />
      <input
        ref={videoPickerRef}
        type="file"
        accept="video/*"
        className="hidden"
        onChange={(e) => handlePickedVideo(e.target.files?.[0])}
      />
    </>
  );
}
...
function TabButton({ active, label, onClick, onHover, icon, badgeCount = 0, hasCustomColor, isSidebarLight }) {
  ...
  const textClass = active
    ? (hasCustomColor
      ? (isSidebarLight ? "text-slate-950 font-bold opacity-100" : "text-white font-bold opacity-100")
      : "text-violet-600 dark:text-violet-400 font-bold opacity-100")
    : (hasCustomColor
      ? (isSidebarLight ? "text-slate-500 hover:text-slate-800 font-normal opacity-70 hover:opacity-100" : "text-white/50 hover:text-white font-normal opacity-60 hover:opacity-100")
      : "text-slate-400 hover:text-slate-700 dark:text-slate-500 dark:hover:text-slate-300 font-normal opacity-70 hover:opacity-100");

  const iconClass = active
    ? "text-violet-600 dark:text-violet-400 scale-[1.05] transition-all duration-200"
    : "text-slate-400 dark:text-slate-500 group-hover:text-slate-600 dark:group-hover:text-slate-350 scale-100 transition-all duration-200";
  ...
  return (
    <button
      ...
      className={`group relative z-10 inline-flex items-center justify-center gap-1.5 rounded-lg px-3 py-1.5 text-xs transition-all duration-200 select-none ${textClass}`}
      aria-pressed={active}
      aria-label={label}
      title={label}
    >
      ...
      <span className="inline-flex items-center gap-1.5">
        {label}
        {showBadge && (
          <span className="inline-flex min-w-4 h-4 items-center justify-center rounded-full bg-rose-500 text-[10px] font-bold leading-none text-white px-1">
