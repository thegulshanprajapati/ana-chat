...
import { Lock, Settings2, Sparkles, X, Keyboard, Link2 } from "lucide-react";
          ...
          <div className="min-h-0 flex-1 space-y-4 overflow-y-auto pr-1">
            ...
            <section className="space-y-2 rounded-xl border border-slate-200 bg-slate-50/80 p-3 dark:border-slate-700 dark:bg-slate-900/70">
              ...
              <button
                ...
                className="mt-2 flex w-full items-center justify-between gap-3 rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-left transition hover:bg-slate-100 dark:border-slate-700 dark:bg-slate-900 dark:hover:bg-slate-800"
              >
                <span className="flex min-w-0 items-start gap-2">
                  <span className="mt-0.5 inline-flex h-8 w-8 items-center justify-center rounded-xl bg-accent-soft text-accent">
                    <Keyboard size={16} />
                  ...
                  <span className="min-w-0">
                    <span className="block text-sm font-medium text-slate-800 dark:text-slate-100">Keyboard Shortcuts</span>
                    <span className="block truncate text-xs text-slate-500 dark:text-slate-400">View and learn app keyboard hotkeys.</span>
                  </span>
                </span>
            ...
            <section className="space-y-2 rounded-xl border border-slate-200 bg-slate-50/80 p-3 dark:border-slate-700 dark:bg-slate-900/70">
              <p className="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">Devices & Security</p>
              <button
                type="button"
              ...
              >
                <span className="flex min-w-0 items-start gap-2">
                  <span className="mt-0.5 inline-flex h-8 w-8 items-center justify-center rounded-xl bg-violet-500/10 text-violet-600 dark:bg-violet-500/20 dark:text-violet-400">
                    <Link2 size={16} />
                  </span>
                  <span className="min-w-0">
                    <span className="block text-sm font-medium text-slate-800 dark:text-slate-100">Linked Devices</span>
                    <span className="block truncate text-xs text-slate-500 dark:text-slate-400">Manage active sessions and link new devices.</span>
                  </span>
                </span>
                <span className="text-xs font-semibold text-slate-600 dark:text-slate-300">Manage</span>
              </button>
            </section>
            ...
            <section className="space-y-2 rounded-xl border border-slate-200 bg-slate-50/80 p-3 dark:border-slate-700 dark:bg-slate-900/70">
              <p className="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">Encrypted Cloud Backup</p>
              <div className="rounded-lg border border-slate-200 bg-white px-3 py-2.5 dark:border-slate-700 dark:bg-slate-900 space-y-3">
                <div className="space-y-1 text-xs">
                  <div className="flex justify-between">
                    <span className="text-slate-500 dark:text-slate-400">Status:</span>
                    <span className="font-semibold text-slate-800 dark:text-slate-200">{backupStatus}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-500 dark:text-slate-400">Backup Size:</span>
                    <span className="font-semibold text-slate-800 dark:text-slate-200">{backupSize}</span>
                  </div>
                  <div className="flex justify-between">
                ...
                <div className="space-y-2">
                  <input
                    ...
                    type="password"
                    placeholder="Enter Backup PIN / Passphrase"
                    className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 outline-none focus:ring-2 focus:ring-violet-500/20 dark:border-slate-700 dark:bg-slate-950/40 dark:text-slate-100"
                  />

                  {backupErr ? <p className="text-xs text-rose-600 dark:text-rose-400">{backupErr}</p> : null}
                  {backupOk ? <p className="text-xs text-emerald-700 dark:text-emerald-300">{backupOk}</p> : null}
                  ...
                  <div className="flex flex-wrap gap-2 pt-1">
                    <button
                      ...
                      onClick={async () => {
                        ...
                        setBackupOk("");
                        if (!backupPin.trim()) {
                          setBackupErr("PIN/Passphrase is required to encrypt backup.");
                          return;
                        }
                        setBackupBusy(true);
                        try {
                          await onCreateBackup?.(backupPin);
                          setBackupOk("Backup created successfully!");
                          setBackupPin("");
                        } catch (err) {
                    ...
                    <button
                      ...
                      onClick={async () => {
                        ...
                        setBackupOk("");
                        if (!backupPin.trim()) {
                          setBackupErr("PIN/Passphrase is required to decrypt backup.");
                          return;
                        }
                        setBackupBusy(true);
                        try {
                          await onRestoreBackup?.(backupPin);
                          setBackupOk("Chats restored successfully!");
                          setBackupPin("");
                        } catch (err) {
                          setBackupErr(err?.message || "Restore failed.");
                        } finally {
                          setBackupBusy(false);
                        }
                      }}
                      className="rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-100 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200 dark:hover:bg-slate-800 disabled:opacity-60"
                    >
                      Restore Backup
                    </button>
                    ...
                    {backupStatus !== "Disabled" ? (
                      <button
                        ...
                        onClick={async () => {
                          ...
                          setBackupBusy(true);
                          try {
                            await onDisableBackup?.();
                            setBackupOk("Backup disabled.");
                            setBackupPin("");
                          } catch (err) {
                            setBackupErr(err?.message || "Operation failed.");
                          } finally {
                            setBackupBusy(false);
                          }
                        }}
                        className="rounded-lg border border-red-200 bg-red-50 px-3 py-1.5 text-xs font-semibold text-red-600 hover:bg-red-100 dark:border-red-900/30 dark:bg-red-950/20 dark:text-red-400 dark:hover:bg-red-900/30 disabled:opacity-60"
                      >
                        Disable & Delete
                      </button>
                    ) : null}
