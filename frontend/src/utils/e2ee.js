...
export async function encryptDatabaseBackup(dbJson, pin) {
  const raw = new TextEncoder().encode(dbJson);
  const salt = crypto.getRandomValues(new Uint8Array(16));
  const iv = crypto.getRandomValues(new Uint8Array(12));
  const key = await deriveKeyFromPin(pin, salt);
  const ciphertext = await crypto.subtle.encrypt({ name: "AES-GCM", iv }, key, raw);
...
export async function decryptDatabaseBackup(ciphertext, salt, iv, pin) {
  ...
  const raw = await crypto.subtle.decrypt({ name: "AES-GCM", iv: ivArr }, key, cipherArr);
  return new TextDecoder().decode(raw);
}

export async function hashBackupPin(pin) {
...
export async function encryptOutgoingMessage({
  ...
  if (hasFile) {
    ...
    const kind = rawMime.startsWith("image/")
      ? "image"
      : (rawMime.startsWith("video/")
        ? "video"
        : (rawMime.startsWith("audio/") ? "audio" : "file"));
