...
export default function ChatPage() {
  ...
  const handleCreateBackup = useCallback(async (pin) => {
    try {
      ...
      notify({ type: "success", message: "Cloud backup completed successfully!" });
    } catch (err) {
  ...
  const handleRestoreBackup = useCallback(async (pin) => {
    try {
      const { data } = await api.get("/messages/backup/download");
      const pinHash = await hashBackupPin(pin);
      
      if (data.backupPinHash !== pinHash) {
        throw new Error("Invalid backup PIN. Decryption rejected.");
      }
      ...
      if (activeChatIdRef.current) {
        await loadMessages(activeChatIdRef.current);
      }

      notify({ type: "success", message: "Chats restored successfully!" });
    } catch (err) {
  ...
  const handleDisableBackup = useCallback(async () => {
    try {
      await api.delete("/messages/backup");
  ...
  }, [decryptChatPreviewForMe, notify, selectChat, setSearch]);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      if (cancelled) return;
      const res = await unlockHiddenChatsWithPin(pin, { silent: true });
      if (res && res.ok) {
        setSearch(""); // Automatically clear the search bar PIN input to restore chat list structure
      }
    }, 260);
