...
export function normalizeChatType(chat: any): "group" | "self" | "direct" {
...
export function directPeerId(chat: any, userId: number | string): number | null {
...
export async function getDirectBlockState(db: Db, userA: number | string, userB: number | string) {
...
export async function getChatMembership(db: Db, chatId: number | string, userId: number | string) {
...
export async function getChatParticipantIds(db: Db, chat: any): Promise<number[]> {
  if (!chat) return [];
  const chatType = normalizeChatType(chat);
  if (chatType === "group") {
...
export async function usersAreConnectedByChat(db: Db, userA: number | string, userB: number | string, chatId: number | string | null = null) {
