...
export default function ChatPage() {
  ...
  const deleteChatById = useCallback(async (chatId) => {
    ...
    setModalConfig({
      type: "confirm",
      title: "Hide Chat Instead?",
      message: "Delete chat is not fully supported yet. Hide chat with PIN instead?",
      onConfirm: () => {
        hideChatById(id);
      }
    });
  ...
  const forwardMessage = useCallback(async (message) => {
    ...
    setModalConfig({
      ...
      message: "Select a chat to forward the message to:",
      candidates,
        ...
        {showRightPane && (
          <div className="min-h-0 min-w-0 h-full panel-in">
            {activeSidebarTab !== "chats" ? (
              // Mobile keeps Calls inside the SidebarPanel (WhatsApp-like top tabs).
              // Desktop swaps the right pane for Calls screens.
              isMobile ? null : (
                <CallsPanel me={user} />
              )
            ) : (
