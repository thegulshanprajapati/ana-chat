...
export async function initSocket(httpServer) {
  ...
  io.on("connection", async (socket) => {
    ...
    // Retrieve and deliver temporary offline message queue from Redis
    try {
      const queueKey = `offline_queue:${userId}`;
      let msgJson;
      while ((msgJson = await redisClient.rPop(queueKey)) !== null) {
        if (msgJson) {
      ...
      console.error("[Socket.IO] Failed to deliver offline messages from Redis:", err.message);
    }
