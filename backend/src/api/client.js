...
api.get = function (url, config) {
  const isDeduplicatedUrl = url === "/auth/me" || url === "/admin/me";
  if (isDeduplicatedUrl) {
    if (activeGetPromises.has(url)) {
      console.log(`[API] Returning active in-flight promise for GET ${url}`);
      return activeGetPromises.get(url);
    }
    const promise = originalGet.call(this, url, config).finally(() => {
      activeGetPromises.delete(url);
    });
    activeGetPromises.set(url, promise);
    return promise;
  }
...
api.interceptors.request.use((config) => {
  ...
  if (isRequestPending(requestKey)) {
    // Keep latest request: cancel the previous in-flight request with same key.
    const previous = pendingRequests.get(requestKey);
    if (previous) previous.abort();
    console.warn("[API] Duplicate request detected, cancelling previous:", requestKey);
...
api.interceptors.response.use(
  ...
  async (error) => {
    ...
    // Don't retry certain requests (including admin requests)
    if (status !== 401 || original._retry || isRefreshRequest || isAdminRequest) {
      if (status === 401) {
    ...
    try {
      ...
      // Don't hard-force logout on transient refresh issues.
      // Let the UI decide (keeps user state until explicit logout).
      log401Detected("/auth/refresh", refreshErr?.response?.data?.message || "Refresh failed");
