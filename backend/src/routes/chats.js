...
async function ensureLockerChat(userId) {
  ...
  const userRow = await db.collection("users").findOne({ id: numericUserId }, { projection: { _id: 0, avatar_url: 1 } });
  const userAvatar = userRow?.avatar_url || "/logo.png";
...
async function getUserChats(userId, options = {}) {
  ...
  const hiddenMessageStateRows = await db.collection("message_user_state").find({
    user_id: me,
    hidden_at: { $ne: null }
  ...
  const rows = chats.map((chat) => {
    ...
    if (chatType === "self") {
      return {
        ...
        other_user_avatar: meUser?.avatar_url || "/logo.png",
...
router.post("/:chatId/clear", requireUser, async (req, res) => {
  const chatId = Number(req.params.chatId);
  if (!chatId) return res.status(400).json({ message: "chatId required" });
  ...
  const messages = await db.collection("messages").find({ chat_id: chatId }, { projection: { id: 1 } }).toArray();
  const now = new Date();

  await Promise.all(messages.map(msg => 
    db.collection("message_user_state").updateOne(
      { user_id: Number(req.user.id), message_id: msg.id },
      {
        $set: { hidden_at: now, chat_id: chatId, updated_at: now },
        $setOnInsert: { user_id: Number(req.user.id), message_id: msg.id, is_starred: false, created_at: now }
      },
      { upsert: true }
    )
  ));
...
router.post("/:chatId/exit", requireUser, async (req, res) => {
  const chatId = Number(req.params.chatId);
  if (!chatId) return res.status(400).json({ message: "chatId required" });
  ...
  await db.collection("chat_members").deleteOne({ chat_id: chatId, user_id: Number(req.user.id) });

  const participantIds = await getChatParticipantIds(db, chat);
  emitChatUpdated(req, [...participantIds, req.user.id], chatId);

  res.json({ success: true });
});

router.delete("/:chatId", requireUser, async (req, res) => {
  const chatId = Number(req.params.chatId);
  if (!chatId) return res.status(400).json({ message: "chatId required" });
  ...
  if (chat.chat_type === "self") {
    return res.status(400).json({ message: "AnaLocker chat cannot be deleted" });
  }
  ...
  // Delete hidden state
  await db.collection("user_hidden_chats").deleteOne({ chat_id: chatId, user_id: Number(req.user.id) });

  // Clear all messages (same as clear chat)
  const messages = await db.collection("messages").find({ chat_id: chatId }, { projection: { id: 1 } }).toArray();
  const now = new Date();
  await Promise.all(messages.map(msg => 
    db.collection("message_user_state").updateOne(
