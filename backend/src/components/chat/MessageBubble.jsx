...
function MessageBubble({
  ...
  const handleInfo = () => {
    const timeStr = new Date(message.created_at || message.timestamp || Date.now()).toLocaleString();
    const details = `Message ID: ${message.id}\nSender: ${message.sender_name || (message.sender_id === meId ? 'You' : 'Partner')}\nTime: ${timeStr}\nStatus: ${message.delivery_status || 'sent'}\nLength: ${(message.body || '').length} characters`;
    alert(details);
    setMenuOpen(false);
  };

  const handlePinMessage = () => {
    const key = `ana_pinned_message_${message.chat_id || 'global'}`;
  ...
  const handleHideMessage = () => {
    ...
    if (!masterPin) {
      ...
      const enterPin = window.prompt("Enter your 4-digit Message PIN to hide this message:");
    ...
    const chatId = message.chat_id || 'global';
    const currentHidden = JSON.parse(localStorage.getItem(`ana_hidden_message_ids_${chatId}`) || "[]");
    if (!currentHidden.includes(message.id)) {
      currentHidden.push(message.id);
      localStorage.setItem(`ana_hidden_message_ids_${chatId}`, JSON.stringify(currentHidden));
      window.dispatchEvent(new Event("ana_hidden_messages_updated"));
    }
    setMenuOpen(false);
    notify?.({ type: "success", message: "Message hidden! 🔒" });
  };
  ...
  useEffect(() => {
    const handleSelectionChange = () => {
      ...
      if (menuRef.current && menuRef.current.contains(selection.anchorNode)) {
        try {
  ...
  return (
    <>
      <div
        className={`w-full flex items-center transition-all duration-200 relative ${isSelectionMode ? "py-1.5 cursor-pointer hover:bg-black/[0.04] dark:hover:bg-white/[0.03]" : ""} ${selected ? "bg-red-500/8 dark:bg-red-500/12 border-y border-red-500/15" : ""}`}
        onClick={(event) => {
          if (isSelectionMode) {
            event.preventDefault();
        ...
        onDoubleClick={() => {
          if (isSelectionMode || deleted || editing) return;
          onReply?.(message);
          setMenuOpen(false);
          setReactionPickerOpen(false);
        }}
      ...
      >
        {isSelectionMode && (
          <div className="flex-shrink-0 mr-3.5 ml-3.5 flex items-center justify-center">
            {selected ? (
              <div className="w-5 h-5 rounded-md bg-red-500 border border-red-600 flex items-center justify-center text-white scale-105 shadow-sm shadow-red-500/20 transition-all duration-150">
                <Check size={13} strokeWidth={3.5} />
              </div>
            ) : (
              <div className="w-5 h-5 rounded-md border-2 border-slate-300 dark:border-slate-600 hover:border-slate-400 dark:hover:border-slate-500 transition-all duration-150" />
            )}
          </div>
        )}

        <div className={`flex-1 flex min-w-0 ${centerAligned ? "justify-center" : (mine ? "justify-end" : "justify-start")}`}>
          <div
          ...
          >
            ...
            {message.reply_to_message_id && !deleted && (
              <div className={`mb-2 rounded-lg border-l-[3.5px] px-3 py-1.5 text-[11px] backdrop-blur-sm transition-all shadow-sm ${
                mine
                  ? "border-white bg-white/15 text-white/95 hover:bg-white/20"
                  : "border-violet-500 bg-slate-100/80 text-slate-700 dark:bg-slate-900/80 dark:text-slate-200 hover:bg-slate-100/90 dark:hover:bg-slate-900/95"
              }`}>
                <div className="flex items-center gap-1.5">
                  <CornerUpLeft size={10} className={mine ? "text-white/85" : "text-violet-500 dark:text-violet-400"} />
                  <p className={`truncate text-[10px] font-bold uppercase tracking-wider ${mine ? "text-white" : "text-violet-600 dark:text-violet-400"}`}>
                    {message.reply_to_sender_name || "Reply"}
                  </p>
                </div>
                <p className={`mt-0.5 truncate text-xs ${mine ? "text-white/80" : "text-slate-500 dark:text-slate-400"}`}>
                  {replyPreview}
                </p>
              </div>
            )}
          ...
          {reactionPickerOpen && !deleted && (
            <>
              ...
              />
              <div className="fixed inset-x-3 bottom-3 z-[120] max-h-[80vh] overflow-y-auto overscroll-contain rounded-[32px] border border-slate-200/80 bg-white/90 p-4 pb-[calc(1rem+env(safe-area-inset-bottom))] shadow-[0_25px_60px_rgba(0,0,0,0.15)] backdrop-blur-2xl transition-all duration-300 dark:border-slate-800/80 dark:bg-slate-950/90 sm:inset-x-0 sm:bottom-6 sm:mx-auto sm:max-w-[480px]">
                {/* Cute Gradient Pull Bar */}
                <div className="mx-auto mb-3 h-1.5 w-12 rounded-full bg-gradient-to-r from-pink-400 via-fuchsia-400 to-indigo-400 opacity-60" />

                <div className="mb-3.5 flex items-center justify-between px-1.5">
                  <p className="text-[12px] font-bold uppercase tracking-[0.16em] bg-gradient-to-r from-pink-500 via-fuchsia-500 to-violet-500 bg-clip-text text-transparent dark:from-pink-400 dark:via-fuchsia-400 dark:to-violet-400">
                    React with love ✨
                  </p>
                  <button
                    type="button"
                    onClick={() => setReactionPickerOpen(false)}
                    className="rounded-full p-1.5 text-slate-400 hover:bg-slate-100 hover:text-slate-800 transition-all active:scale-90 dark:text-slate-500 dark:hover:bg-slate-900 dark:hover:text-slate-200"
                    aria-label="Close reactions"
                  >
                    <X size={15} />
                  </button>
                </div>
                <div className="grid grid-cols-6 gap-2.5 px-0.5 mb-2">
                  {REACTION_PICKER.map((reaction) => (
                    <button
                      key={reaction}
                      type="button"
                      onClick={() => handleReactionSelect(reaction)}
                      className={`inline-flex h-11 items-center justify-center rounded-2xl border text-xl transition-all duration-300 ease-out hover:scale-125 hover:rotate-3 active:scale-90 ${
                        message.my_reaction === reaction
                          ? "border-pink-400 bg-pink-50/50 shadow-[0_0_15px_rgba(244,63,94,0.25)] dark:border-pink-500/50 dark:bg-pink-500/10"
                          : "border-slate-100 bg-slate-50/70 hover:border-pink-300 hover:bg-pink-50/20 dark:border-slate-800 dark:bg-slate-900/50 dark:hover:border-violet-500/30 dark:hover:bg-violet-950/10"
                      }`}
                      aria-label={`React ${reaction}`}
                    >
                      <span className="text-[23px] leading-none filter drop-shadow-[0_2px_4px_rgba(0,0,0,0.08)] transform active:scale-95 transition-transform">{reaction}</span>
                    </button>
                  ))}
                </div>
                ...
                <div className="mt-3.5 flex flex-col gap-2.5">
                  <button
                    ...
                    className="inline-flex w-full items-center justify-center gap-2 rounded-full border border-slate-200/80 bg-white/50 px-3 py-2 text-xs font-bold text-slate-700 transition-all duration-300 hover:border-pink-400/50 hover:bg-pink-50/10 active:scale-98 dark:border-slate-800 dark:bg-slate-900/50 dark:text-slate-200 dark:hover:border-violet-500/40 dark:hover:bg-violet-950/10"
                  >
                    <SmilePlus size={15} className="text-pink-500 dark:text-violet-400" />
                    {emojiPickerOpen ? "Hide Emojis 🌸" : "More Emojis 💖"}
                  </button>

                  {emojiPickerOpen && (
                    <div className="overflow-hidden rounded-[24px] border border-slate-200/80 shadow-inner dark:border-slate-800 transition-all duration-300">
                      <EmojiPicker
                        width="100%"
                        height={320}
                        lazyLoadEmojis
                  ...
                  {message.my_reaction && (
                    <button
                      ...
                      className="w-full rounded-full border border-rose-200 bg-rose-50/50 px-3 py-2 text-xs font-bold text-rose-600 transition-all duration-300 hover:bg-rose-100/50 active:scale-98 dark:border-rose-950 dark:bg-rose-950/20 dark:text-rose-400 dark:hover:bg-rose-950/45"
                    >
                      Remove My Reaction 💔
                    </button>
                  )}
          ...
          {menuOpen && typeof document !== "undefined" && createPortal((
            <>
              ...
              <div
                ref={mobileMenuRef}
                onContextMenu={(event) => event.preventDefault()}
                className="fixed inset-x-0 bottom-0 z-[120] max-h-[74vh] overflow-y-auto overscroll-contain rounded-t-[32px] border-t border-slate-200/80 bg-white/95 px-5 pt-4 pb-[calc(1.5rem+env(safe-area-inset-bottom))] shadow-[0_-15px_40px_rgba(0,0,0,0.12)] backdrop-blur-xl dark:border-slate-800/80 dark:bg-slate-950/95 sm:hidden"
              >
                {/* Drag handle / Pull bar */}
                <div className="mx-auto mb-5 h-1.5 w-12 rounded-full bg-gradient-to-r from-pink-300 via-fuchsia-300 to-indigo-300 opacity-60" />
              ...
              <div
                ...
                className="chat-menu-in hidden sm:block sm:fixed sm:z-[120] sm:w-56 sm:max-h-[calc(100vh-24px)] sm:overflow-y-auto sm:overscroll-contain sm:rounded-[24px] sm:border sm:border-slate-200/80 sm:bg-white/95 sm:p-2 sm:shadow-[0_20px_50px_rgba(0,0,0,0.15)] sm:backdrop-blur-xl dark:sm:border-slate-800/80 dark:sm:bg-slate-900/95"
              >
      ...
      {selectionCoords && selectedText && createPortal(
        <div
          style={{
            position: "absolute",
            top: `${selectionCoords.top - 45}px`,
            left: `${selectionCoords.left}px`,
            transform: "translateX(-50%)",
          }}
          className="z-[9999] flex items-center gap-1 rounded-full border border-slate-200/50 bg-slate-950/80 px-2 py-1 text-xs text-white shadow-xl backdrop-blur-md dark:border-slate-800/50 animate-in fade-in zoom-in-95 duration-100"
        >
          <button
            type="button"
            onClick={async (e) => {
            ...
            className="flex items-center gap-1.5 rounded-full px-2.5 py-1 transition hover:bg-white/10"
          >
            <Copy size={12} />
            <span>Copy</span>
          </button>
...
function MenuButton({ icon, label, className = "", onClick, disabled = false }) {
  const isDestructive = className.includes("rose") || className.includes("danger") || label.toLowerCase().includes("delete");
  
  const hoverClass = isDestructive
    ? "hover:bg-rose-50 hover:text-rose-600 dark:hover:bg-rose-950/35 dark:hover:text-rose-300"
    : "hover:bg-violet-50 hover:text-violet-600 dark:hover:bg-violet-950/35 dark:hover:text-violet-300";
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className={`flex w-full items-center gap-3 rounded-xl border border-transparent px-3 py-2 text-[13.5px] font-semibold text-slate-700 transition-all duration-200 hover:scale-[1.02] active:scale-97 disabled:cursor-not-allowed disabled:opacity-50 dark:text-slate-200 ${hoverClass} ${className}`}
    >
      <span className="opacity-75 transition-colors">{icon}</span>
      <span>{label}</span>
    </button>
  );
}
