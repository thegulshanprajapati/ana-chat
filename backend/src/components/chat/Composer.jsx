...
export default function Composer({
  ...
  const syncComposerHeights = () => {
    ...
    input.style.height = `${Math.max(nextHeight, 40)}px`;
  ...
  return (
    ...
    >
      ...
      {replyTo && (
        <div className="mb-2 flex items-start justify-between gap-3 rounded-xl border border-violet-500/20 bg-violet-500/5 dark:bg-violet-950/20 px-3.5 py-2.5 text-xs shadow-sm backdrop-blur-sm">
          <div className="min-w-0 flex-1">
            <p className="inline-flex items-center gap-1.5 font-semibold text-violet-600 dark:text-violet-400">
              <CornerUpLeft size={13} strokeWidth={2.5} />
              Replying to {replyName}
            </p>
            <p className="mt-1 max-h-9 overflow-hidden break-words text-slate-600 dark:text-slate-300 text-[11px] leading-relaxed truncate">{replyPreview}</p>
          </div>
          <button
            type="button"
            onClick={onCancelReply}
            className="rounded-full p-1.5 text-slate-400 hover:bg-slate-200/50 hover:text-slate-700 dark:text-slate-500 dark:hover:bg-white/5 dark:hover:text-slate-200 active:scale-90 transition-all duration-150 shrink-0"
      ...
      <div className="flex items-end gap-2 w-full">
        ...
        {/* Textarea Area */}
        <div className="relative flex-1 min-h-[40px] self-center">
          <div
            ref={overlayRef}
            aria-hidden
            className="pointer-events-none absolute inset-0 z-0 min-h-[40px] overflow-hidden px-2 py-[10px] text-[15px] leading-5 text-slate-900 outline-none whitespace-pre-wrap break-words dark:text-slate-50"
          ...
          <textarea
            ...
            placeholder={disabled
              ...
              : "Message"}
            rows={1}
            className="relative z-10 min-h-[40px] w-full resize-none bg-transparent px-2 py-[10px] text-[15px] leading-5 text-transparent caret-slate-900 outline-none placeholder:text-slate-500/90 disabled:cursor-not-allowed disabled:placeholder:text-slate-400 dark:caret-slate-50 dark:placeholder:text-white/45"
        ...
        {canSend ? (
          ...
          <button
            type="button"
            onClick={() => {
              notify?.({ type: "info", title: "Voice Notes", message: "Voice messages feature coming soon!" });
            }}
            className="flex h-[42px] w-[42px] shrink-0 items-center justify-center rounded-full bg-[#00a884] dark:bg-[#00a884] text-white shadow-md transition duration-150 hover:scale-105 active:scale-95 focus:outline-none focus:ring-2 focus:ring-[#00a884]/30"
