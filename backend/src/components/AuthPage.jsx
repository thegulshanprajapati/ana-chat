...
export default function AuthPage({ onAuthed }) {
  ...
  if (user) {
    return (
      <div className="min-h-screen w-full bg-gradient-to-b from-[#050816] to-[#0B1020] flex flex-col items-center justify-center p-4 relative overflow-hidden font-sans text-white">
        ...
        <div className="relative z-10 max-w-md w-full bg-[#0B1020]/45 backdrop-blur-2xl border border-white/10 rounded-[28px] p-8 md:p-10 shadow-[0_30px_70px_rgba(0,0,0,0.5)] text-center">
          <div className="relative flex items-center justify-center w-16 h-16 rounded-full bg-violet-500/10 border border-violet-500/20 shadow-[0_0_25px_rgba(139,92,246,0.2)] mx-auto mb-6">
          ...
          <p className="text-sm text-slate-400 mb-6 leading-relaxed">
            You are signed in as <span className="text-white font-semibold">{user.name || user.email}</span>.
          </p>
  ...
  async function handleLogin(e) {
    ...
    try {
      ...
      setTimeout(() => {
        if ((data?.mode === "admin" && data?.admin?.id) || data?.isAdmin) {
          navigateTo("admin");
          return;
        }
        onAuthed();
  ...
  async function handleLogoutAllDevices(options = {}) {
    ...
    try {
      ...
      if (deleteChats) {
        const ok = window.confirm("This will hide/delete ALL your chats on this account (for you). This cannot be easily undone. Continue?");
        if (!ok) return;
      }
