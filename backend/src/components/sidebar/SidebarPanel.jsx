...
export default function SidebarPanel({
  ...
  const tabOrder = useMemo(() => ["chats", "calls"], []);
  ...
  return (
    <>
      ...
      >
        ...
        >
          <div
            className={`relative grid grid-cols-2 rounded-full border p-[2px] ${
            hasCustomSidebarColor
              ? (isSidebarLight ? "border-slate-200 bg-white/70" : "border-white/20 bg-white/10")
              : "border-slate-200/80 bg-slate-100/80 dark:border-slate-800/80 dark:bg-slate-900/50"
          }`}>
            <div
              aria-hidden
              className="pointer-events-none absolute inset-y-[2px] rounded-full bg-violet-600 dark:bg-violet-500 shadow-[0_2px_8px_rgba(124,58,237,0.25)]"
              style={{
                left: activeTabIndex === 0 ? "2px" : "50%",
                right: activeTabIndex === 0 ? "50%" : "2px",
                transitionProperty: "left, right",
                transitionDuration: "360ms, 360ms",
                transitionTimingFunction: "cubic-bezier(0.25, 1, 0.2, 1.08)",
                transitionDelay: activeTabIndex === 0 ? "0ms, 45ms" : "45ms, 0ms"
              }}
            />
            <TabButton
...
function TabButton({ active, label, onClick, onHover, icon, badgeCount = 0, hasCustomColor, isSidebarLight }) {
  ...
  return (
    <button
      ...
      className={`relative z-10 inline-flex items-center justify-center gap-1.5 rounded-full px-2.5 py-2 text-xs font-semibold transition duration-200 ${
        active
          ? "text-white"
          : (hasCustomColor
            ? (isSidebarLight
