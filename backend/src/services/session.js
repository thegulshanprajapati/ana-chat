...
export async function isKnownDeviceForRequest(userId, req) {
  const fingerprint = req?.headers?.["x-device-fingerprint"] || null;
  if (!fingerprint) return false;
  const existing = await findActiveSessionByFingerprint(Number(userId), fingerprint);
...
export async function createSessionForUser(userId, req) {
  ...
  const db = await getDb();
  const existingSession = await findActiveSessionByFingerprint(normalizedUserId, meta.fingerprint);
  let sessionId;

  if (existingSession) {
    sessionId = existingSession.id;
  } else {
    const activeSessions = await getActiveSessions(normalizedUserId);
    // MAX_ACTIVE_DEVICES <= 0 disables device limiting (unlimited logins)
    if (MAX_ACTIVE_DEVICES > 0 && activeSessions.length >= MAX_ACTIVE_DEVICES) {
  ...
  const refreshToken = signRefreshToken(normalizedUserId, sessionId);
  ...
  const accessToken = signAccessToken(normalizedUserId, sessionId);
...
export async function rotateRefreshSession({ sessionId, userId, currentRefreshToken, req }) {
  const db = await getDb();
  const normalizedUserId = Number(userId);
  const sessionRow = await db.collection("sessions").findOne({ id: sessionId, user_id: normalizedUserId, revoked_at: null });
  if (!sessionRow) return null;

  if (sessionRow.refresh_token_hash !== sha256(currentRefreshToken)) return null;

  const nextRefreshToken = signRefreshToken(normalizedUserId, sessionId);
  ...
  const nextAccessToken = signAccessToken(normalizedUserId, sessionId);
...
export async function revokeAllUserSessions(userId) {
  ...
  const normalizedUserId = Number(userId);
  await db.collection("sessions").updateMany(
    { user_id: normalizedUserId, revoked_at: null },
    { $set: { revoked_at: new Date() } }
  );
}
